...
library(castor)
# Read the tree
tree_files <- list.files(pattern = "^72.*substituted_tumor_treev2\\.3\\.0\\.nwk$")
...
distance_type$Estimated_node_timing_Median <- (26*7+20)*(3.4-distance_type$Mean_MB/median_M0)/2.4
...
write.table(df_all, "72_combined_trimed_Np.tsv", sep="\t", row.names=FALSE, na = "NA",quote=F)

write.table(timing_all, "72_combined_trimed_timing.tsv", sep="\t", row.names=FALSE, na = "NA",quote=F)
